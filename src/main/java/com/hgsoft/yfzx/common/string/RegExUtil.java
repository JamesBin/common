...
public class RegExUtil {
    ...
    /**
     * 功能描述：主要用于模板中模块标记分析函数 把查找到的元素加到vector中
     * pattern为正则表达式模式
     *
     * @param
     * @param str 原始字串
    ...
    /**
     * 功能描述：匹配所有符合模式要求的字串并加到矢量vector数组中
     * pattern为正则表达式模式
     *
     * @param
     * @param str 原始字串
    ...
    /**
     * 功能描述：转义正则表达式字符(之所以需要将\和$字符用escapeDollarBackslash方法的方式是因为用repalceAll是不行的，简单的试试"$".repalceAll("\\$","\\\\$")你会发现这个调用会导致数组越界错误)
     * pattern为正则表达式模式
     *
     * @param original 原始字串
     * @param
